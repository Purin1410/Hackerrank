 ...
 */
int calDistance(int fruit_pos, int tree_loc, int s, int t) {
    int fruit_loc = fruit_pos + tree_loc;
    if (s<= fruit_loc and fruit_loc <= t) {
        return 1;
    }
    return 0;
}
...
void countApplesAndOranges(int s, int t, int a, int b, vector<int> apples, vector<int> oranges) {
    ...
    int count_oranges = 0;
    int i = 0;
    while(i < apples.size() or i < oranges.size()) {
        if (i < apples.size()) count_apples += calDistance(apples[i], a, s, t);
        if (i < oranges.size()) count_oranges += calDistance(oranges[i], b, s, t);
        i ++;
    }
